...
- **Location tags:** synodic sites / comets, centaurs, the Jovian Greek /
  ...
  (`data/ad-astra.js`) are the real map nodes (node-tags `exit: true` for the
  exits; the two sunlenses by slug). They were zone-edge locations until
  2026-09-23, which was too strict, too loose and mis-scored the lenses.
- **EPIC_HAZARD op (1A6):** promoted card + a colocated Human (a Human
  ...
  and is DECOMMISSIONED per 1D1b: patents return to the owner's hand, fuel
  cargo is destroyed, colonists requeue + exomigrate replacements, crew
  restarts at LEO). Until 2026-09-23 the patents went to the bottom of their
  decks instead; the admin Manage state panel's "Cards lost at op #" lookup
  finds cards an op buried that way and hands them back.
