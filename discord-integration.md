...
- `server/discord.js` - REST DM sender (`sendDM`, `discordEnabled`) and the
  OAuth helpers (`oauthEnabled`, `oauthClientId`, `buildAuthorizeUrl`,
  `completeOauth` = code exchange -> identify -> guilds.join). Inert
  without the matching env vars.
- `server/db.js` - `notify_prefs` table (per-profile opt-in).
- `server/index.js` - `GET/PUT /me/notify`, `POST /me/notify/test`,
  `POST /me/notify/oauth/start` (returns the authorize URL), and
  `GET /auth/discord/callback` (server-side link + confirmation DM).
  `dispatchTurnNotifications()` runs after each committed op. The OAuth
  `state` is held in memory with a 10-minute TTL.
- `js/api.js` - `getNotifyPrefs` / `setNotifyPrefs` / `testNotify` /
  `startDiscordOauth`.
- `js/main.js` + `index.html` - the "Turn notifications" menu: the Connect
  Discord button (popup + poll), the event checkboxes, and the manual
  user-ID fallback disclosure.
