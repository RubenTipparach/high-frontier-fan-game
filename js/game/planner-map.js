...
// Data URLs resolved from the app root via base.js (bundling-safe). The
// visible URL can be a deep /room/<CODE> path, so an address-bar-relative
// './data/...' would resolve to /room/data/... (404) and the map fails.
const PLANNER_JSON_URL = assetUrl('vendor/hf-mission-planner/assets/data-hf4.json');
...
const SITE_FLAGS_URL = assetUrl('data/site-flags.json');
...
// Id + name-match rules live in data/ (shared) so the slug this client
// stamps is byte-for-byte the slug the server's planner-graph builds.
...
// id2 (the node's stable, shared reference) is generated by makeRefId
// from data/planner-ids.js - imported above so this client and the
// server's planner-graph stamp the SAME slug. disambiguateRefIds only
// suffixes the rare post-rule collision.
