...
class Heap {
  ...
  moveDown_(index) {
    ...
    while (index < (count >> 1)) {
      ...
      var smallerChildIndex = rightChildIndex < count &&
              lt(nodes[rightChildIndex].getKey(), nodes[leftChildIndex].getKey()) ?
          rightChildIndex :
          leftChildIndex

      // If the node being moved down is smaller than its children, the node
      // has found the correct index it should be at.
      if (lt(node.getKey(), nodes[smallerChildIndex].getKey())) {
        break
      }
  ...
  moveUp_(index) {
    ...
    while (index > 0) {
      ...
      if (lt(node.getKey(), nodes[parentIndex].getKey())) {
...
// Ported to ES modules for this codebase. Source: vendor/hf-mission-planner/src/heap.js.
export default Heap;
